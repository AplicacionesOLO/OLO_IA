# ═══════════════════════════════════════════════════════════════════════════
#  ESCALAMIENTO DE INCIDENCIAS VENCIDAS, COMO TAREA PROGRAMADA
#
#      powershell -ExecutionPolicy Bypass -File tools\escalar_incidencias_servicio.ps1 -Instalar
#
#  Mismo patron que vigilante_servicio.ps1: un barrido corto (unas pocas
...
#  Y el mismo motivo para usar `backend\.venv`: este guion solo habla con la
#  API por `urllib`, biblioteca estandar — no toca video ni un modelo.
# ═══════════════════════════════════════════════════════════════════════════
...
$TAREA   = 'OLO_IA - Escalamiento de incidencias vencidas'
$RAIZ    = Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path)
$PYTHON  = Join-Path $RAIZ '.venv\Scripts\python.exe'
$SCRIPT  = Join-Path $RAIZ 'tools\escalar_incidencias.py'
$LOGDIR  = Join-Path $env:LOCALAPPDATA 'OLO_IA'
$LOG     = Join-Path $LOGDIR 'escalar_incidencias.log'
...
function Requisitos {
    ...
    if (-not (Test-Path $clave)) {
        throw "Falta la contraseña en $clave. Sin ella el escalamiento no puede autenticarse."
    }
...
if ($Instalar) {
    ...
    Register-ScheduledTask -TaskName $TAREA -Action $accion -Trigger $disparador `
        ...
        'Avisa a quien tiene asignada una incidencia de que su plazo (due_date) ya paso. Ver tools/escalar_incidencias_servicio.ps1' `
        -Force | Out-Null

    Write-Output "Instalada: «$TAREA», cada $CadaMinutos minutos"
    Write-Output "  API   : $Api"
    Write-Output "  log   : $LOG"
    Write-Output "  parar : powershell -File tools\escalar_incidencias_servicio.ps1 -Desinstalar"
    exit 0
}
